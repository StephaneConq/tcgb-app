<script>
    ...
    function increment() {
        api.patch(`/api/collection`, {
            cards: [
                card.card_versions ? card.card_versions[0] : card
            ]
        }).then(() => {
            card.count += 1;
        });
    ...
    function decrement() {
        api.delete(`/api/collection?card_ref=${card.card_versions ? card.card_versions[0].ref : card.ref}`).then(() => {
            card.count -= 1;
        });
    }
</script>
...
>
    ...
    <div class="flex flex-col gap-3 border-t border-gray-200 p-4 dark:border-zinc-700 flex-1">
        ...
        <div class="flex justify-between text-xs text-gray-500 dark:text-zinc-400">
            <span>Set: {card.set_id}</span>
            <span>No. {card.card_number}</span>
        </div>
