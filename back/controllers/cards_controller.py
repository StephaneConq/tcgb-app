...
**Your primary objective is to extract the unique identifying information needed to easily find a specific card in an online database, specifically the set tag and the card number.**

---

**1. Information to Extract:**

*  **Set Tag (Required):** The alphanumeric code representing the card's set.
...
*  **Card Number (Required):** The specific number of the card within its set.
  *  **Location:** Directly adjacent to or very close to the Set Tag, often part of a fraction.
...
*  **Card Name (Optional, for verification):** The official name of the Pokémon, Trainer, or Energy card.
  ...
  *  **Purpose:** To help confirm the correct card if there's any ambiguity with the set tag or card number.
...
```json
{
  \"set_tag\": \"VALUE_OF_SET_TAG\",
  \"card_number\": \"VALUE_OF_CARD_NUMBER_X\",
  \"card_name\": \"VALUE_OF_CARD_NAME_IF_EXTRACTED\"
}
```

*  **`set_tag`**: The extracted set tag (e.g., \"SV6\"). If not found, use `null`.
...
*  **Contextual Clues:** The set tag and card number are almost always together at the very bottom of the card.
*  **Fraction Format:** When you see `X/Y`, the `X` is the card number you need. Disregard `Y` for the primary lookup.
*  **Handle Missing Data:** If you cannot confidently identify either the set tag or the card number, set its respective field to `null` and provide details in the `notes` field, along with a lower `confidence_score`.
...
```"""
    ...
    generate_content_config = types.GenerateContentConfig(
        ...
        response_schema={"type": "ARRAY", "items": {"type": "OBJECT", "properties": {"set_tag": {
